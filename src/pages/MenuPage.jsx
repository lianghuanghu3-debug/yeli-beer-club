...
export default function MenuPage({ active, onWheel }) {
  ...
  return (
    <section
      ref={ref}
      className="relative w-full h-screen flex items-center bg-gradient-to-b from-[#060f06] via-[#0a1408] to-[#060f06] overflow-hidden"
    >
      {/* Grass */}
      <div className="absolute bottom-0 left-0 right-0 h-[15vh] pointer-events-none opacity-[0.04]">
        {[...Array(80)].map((_, i) => (
          <div key={i} className="absolute bottom-0 w-px bg-[#4a8c5a]"
            style={{ left: `${(i/80)*100}%`, height: `${15+Math.random()*50}px`, transform: `rotate(${(Math.random()-0.5)*10}deg)` }} />
        ))}
      </div>

      <div className="relative z-10 w-full max-w-3xl mx-auto px-4 md:px-12 overflow-y-auto max-h-screen py-10">
        <div className={`transition-all duration-1000 ${active ? "opacity-100" : "opacity-0 translate-y-6"}`}>
          ...
          <div className="grid md:grid-cols-2 gap-4 md:gap-6">
            {/* ¥9.9 */}
            <div className={`group relative rounded-2xl border border-white/[0.06] bg-gradient-to-b from-white/[0.02] to-transparent p-6 md:p-8 transition-all duration-700 hover:border-amber-400/20 ${show ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
              <div className="absolute -top-16 -right-16 w-32 h-32 rounded-full bg-amber-400/5 blur-3xl pointer-events-none" />
            ...
            {/* ¥39.9 */}
            <div className={`group relative rounded-2xl border border-amber-400/15 bg-gradient-to-b from-amber-400/[0.03] to-transparent p-6 md:p-8 transition-all duration-700 hover:border-amber-400/30 ${show ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}
              style={{ transitionDelay: "300ms" }}>
              <div className="absolute -top-16 -right-16 w-48 h-48 rounded-full bg-amber-400/8 blur-3xl pointer-events-none" />
